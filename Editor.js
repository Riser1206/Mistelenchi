...
function clickCanvas() {
    ...
    if (X == -1 || Y == -1 || X == Length_ || Y == Length_) return (Euameate=document.createElement('ocle')).innerText = JSON.stringify({PathSwitches:PathSwitches,Switches:Switches,Paths:Paths,Board:Board,Solve:Solve,isPathNum:isPathNum}, null, 2,alert(Euameate),alert(document.getElementById('ocle')))
    if (isPathNum) {
        let $$
