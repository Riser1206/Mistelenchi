...
try{(Euameate=document.createElement('textarea')).style.fontSize = 300
...
function clickCanvas() {
    ...
    if (X == -1 || Y == -1 || X == Length_ || Y == Length_) return Euameate.innerText = JSON.stringify({PathSwitches:PathSwitches,Switches:Switches,Paths:Paths,Board:Board,Solve:Solve,isPathNum:isPathNum}, null, 2)
    if (isPathNum) {
        let $$
