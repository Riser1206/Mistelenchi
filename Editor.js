...
function select() {
    ...
    PathSwitches.forEach((PathSwitch, i) => {
        ...
        if ($.isPointInStroke(PathSwitch,Math.round((event.clientX-$.canvas.offsetLeft-5)*_/($.canvas.offsetWidth-10)-.5),Math.round((event.clientY-$.canvas.offsetTop-5)*_/($.canvas.offsetHeight-10)-.5))) isPathNum = i+1
    })
    return isPathNum
}
