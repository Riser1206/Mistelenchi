...
function clickCanvas() {
    ...
    if (isPathNum) {
    try {
        if ($.isPointInPath({ type: 'text/plain' },Math.round((event.clientX-$.canvas.offsetLeft-5)*_/($.canvas.offsetWidth-10)-.5),Math.round((event.clientY-$.canvas.offsetTop-5)*_/($.canvas.offsetHeight-10)-.5))) alert(JSON.stringify(Switches))
