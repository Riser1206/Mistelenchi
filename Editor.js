...
function clickCanvas() {
    ...
    if (X == -1 || Y == -1 || X == Length_ || Y == Length_) return document.createElement('ocumeaateEle').innerText = JSON.stringify({PathSwitches:PathSwitches,Switches:Switches,Paths:Paths,Board:Board,Solve:Solve,isPathNum:isPathNum}, null, 2,alert(`귝 그`),alert(document.getElementById('ocumeaateEle')))
    if (isPathNum) {
        let $$
