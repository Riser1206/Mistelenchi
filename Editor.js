...
function clickCanvas() {
    ...
    if (isPathNum) {
    try {
        if ($.isPointInPath(Switches[X][Y],Math.round((event.clientX-$.canvas.offsetLeft-5)*_/($.canvas.offsetWidth-10)-.5),Math.round((event.clientY-$.canvas.offsetTop-5)*_/($.canvas.offsetHeight-10)-.5))) alert(JSON.stringify(Switches))
    } catch (ium) {Qnumber.style.fontSize=3;Qnumber.innerText = ium+Switches[X][Y]}
        isPathNum = 0
    } else if (isPathNum == []) {
