...
function clickCanvas() {
    ...
    if (isPathNum) {
    let pushpop //try{Qnumber.innerText=JSON.stringify(Paths.splice(isPathNum-1))}catch(thN){Qnumber.innerText=thN}UR
        for (let y = 0; y < 3; y++) for (let x = 0; x < 3; x++) if ($.isPointInPath(Switches[x][y],Math.round((event.clientX-$.canvas.offsetLeft-5)*_/($.canvas.offsetWidth-10)-.5),Math.round((event.clientY-$.canvas.offsetTop-5)*_/($.canvas.offsetHeight-10)-.5))) {
            if (x*y-1) pushpop = Paths[isPathNum-1].Array.push(4-Math.atan2(x-1,y-1)/Math.PI*4); else pushpop = Paths[isPathNum-1].Array.pop()
        }
     try{alert(JSON.stringify(Paths)+"\n"+pushpop)}catch(thN){Qnumber.innerText=thN}
