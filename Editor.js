...
function clickCanvas() {
    ...
    if (X == -1 || Y == -1 || X == Length_ || Y == Length_) return Euameate.value = JSON.stringify({isPathNum:isPathNum,Color:Color,PathSwitches:PathSwitches,Switches:Switches,Paths:Paths,Board:Board,Solve:Solve}, null, 2)
    if (isPathNum) {
        let $$
        ...
        if (select()) alert(); else isPathNum = Paths.push({Color:Color,X:X+1,Y:Y+1,Array:[]})
