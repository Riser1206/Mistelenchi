...
innerWidth > innerHeight ? spurn("가로 화면입니다. 세로 화면으로 바꿔주세요.",0); else !Number.isInteger(Level[2] -= 0) || Level[2] < 1 ? spurn(Level[2],1) : fetch(`Levels/${Level[1]}/${Level[2]}.json`).then(res => res.json()).then(loadMap).catch($ => spurn($,9))
